...
const BlogDetail = () => {
    ...
    const fetchBlogDetails = async () => {
      try {
        ...
        const response = await fetch(
          `https://fields-garcia-developed-consider.trycloudflare.com//api/blogs/${id}`
        );
        if (!response.ok) {
  ...
  const fetchRelatedBlogs = async (category) => {
    try {
      const response = await fetch(
        `https://fields-garcia-developed-consider.trycloudflare.com//api/blogs?page=1&limit=3`
      );
      if (response.ok) {
        const data = await response.json();
